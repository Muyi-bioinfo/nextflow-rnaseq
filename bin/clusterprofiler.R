...
for (i in seq_along(sig_files)) {
  ...
  # 基因 ID → ENTREZID 转换 (容错：keytype 不匹配时优雅跳过)
  entrez_res <- tryCatch(
    bitr(gene_ids, fromType = from_type, toType = "ENTREZID", OrgDb = org_db),
    error = function(e) {
      warning(sprintf("  bitr() 失败: %s", conditionMessage(e)))
      data.frame()
    }
  )
  genes_entrez <- if (nrow(entrez_res) > 0) unique(entrez_res$ENTREZID) else character(0)
